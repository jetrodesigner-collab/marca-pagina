...
export default function ItemDetail({ session, item, userItem, onBack, onUserItemUpdate }) {
  const [theme]      = useState(() => localStorage.getItem('tema') || 'D')
  const [activeTab,  setActiveTab]  = useState('R')
  const [status,     setStatus]     = useState(userItem.status)
  const [rating,     setRating]     = useState(userItem.rating || 0)
  const [review,     setReview]     = useState('')
  const [isPublic,   setIsPublic]   = useState(false)
  const [saving,     setSaving]     = useState(false)
  const [saveFeedback, setSaveFeedback] = useState(null) // 'ok' | 'err'
  ...
  async function changeStatus(newStatus) {
    ...
    if (error) {
      ...
      onUserItemUpdate?.({ ...userItem, status: newStatus, rating })
    }
  }
  ...
  async function changeRating(stars) {
    ...
    await supabase
      ...
      .eq('id', userItem.id)
    onUserItemUpdate?.({ ...userItem, status, rating: newRating })
  }
  ...
  async function saveReview() {
    ...
    const { error } = await supabase
      .from('reviews')
      .upsert(
        { user_id: session.user.id, item_id: item.id, body: review, is_public: isPublic, updated_at: new Date().toISOString() },
        ...
        <div className="sc">
          ...
          <div className="hero">
            ...
            <div className="hm">
              ...
              {item.author   && <div className="hs">{item.author}</div>}
              {item.director && <div className="hs">Dir. {item.director}</div>}
              {item.year     && <div className="hy">{item.year}</div>}
